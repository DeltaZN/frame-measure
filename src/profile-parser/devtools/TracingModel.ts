...
export class TracingModel {
    ...
    readonly #devToolsMetadataEventsInternal: TracingModelEvent[];
    ...
    readonly #mainFrameNavStartTimes: Map<string, TracingModelEvent>;
    ...
    static isTopLevelEvent(event: TracingModelEvent): boolean {
    ...
    static browserMainThread(tracingModel: TracingModel): Thread|null {
        ...
        const browserProcesses: Process[] = [];
        const browserMainThreads: Thread[] = [];
    ...
    devToolsMetadataEvents(): TracingModelEvent[] {
        ...
        if (payload.name === 'navigationStart') {
            ...
            if (data) {
                ...
                if (isLoadingMainFrame && documentLoaderURL !== '') {
                    ...
                    const navStartEvent = TracingModelEvent.fromPayload(payload, thread);
    ...
    private addSampleEvent(event: TracingModelEvent): void {
    ...
    profileGroup(event: TracingModelEvent): ProfileEventsGroup|null {
    ...
    extractEventsFromThreadByName(processName: string, threadName: string, eventName: string): TracingModelEvent[] {
    ...
    private processPendingAsyncEvents(): void {
        this.#asyncEvents.sort(TracingModelEvent.compareStartTime);
    ...
    private addNestableAsyncEvent(event: TracingModelEvent): void {
        const phase = Phase;
    ...
    private addAsyncEvent(event: TracingModelEvent): void {
        const phase = Phase;
...
export class TracingModelEvent {
    ...
    static fromPayload(payload: EventPayload, thread: Thread): TracingModelEvent {
        const event = new TracingModelEvent(payload.cat, payload.name, (payload.ph as Phase), payload.ts / 1000, thread);
        if (payload.args) {
    ...
    static compareStartTime(a: TracingModelEvent|null, b: TracingModelEvent|null): number {
        if (!a || !b) {
            return 0;
        }
    ...
    static orderedCompareStartTime(a: TracingModelEvent, b: TracingModelEvent): number {
    ...
    complete(endEvent: TracingModelEvent): void {
        if (endEvent.args) {
...
export class ObjectSnapshot extends TracingModelEvent {
...
// eslint-disable-next-line rulesdir/enforce_custom_event_names, rulesdir/static_custom_event_names
export class AsyncEvent extends TracingModelEvent {
    steps: TracingModelEvent[];
    causedFrame: boolean;

    constructor(startEvent: TracingModelEvent) {
    ...
    addStep(event: TracingModelEvent): void {
...
class ProfileEventsGroup {
    ...
    addChild(event: TracingModelEvent): void {
...
export class Process extends NamedObject {
    ...
    addEvent(payload: EventPayload): TracingModelEvent|null {
...
export class Thread extends NamedObject {
    ...
    #eventsInternal: TracingModelEvent[];
    readonly #asyncEventsInternal: AsyncEvent[];
    #lastTopLevelEvent: TracingModelEvent|null;
    ...
    tracingComplete(): void {
        this.#asyncEventsInternal.sort(TracingModelEvent.compareStartTime);
        this.#eventsInternal.sort(TracingModelEvent.compareStartTime);
        const phases = Phase;
    ...
    addEvent(payload: EventPayload): TracingModelEvent|null {
        const event = payload.ph === Phase.SnapshotObject ? ObjectSnapshot.fromPayload(payload, this) :
            TracingModelEvent.fromPayload(payload, this);
    ...
    removeEventsByName(name: string): TracingModelEvent[] {
        const extracted: TracingModelEvent[] = [];
