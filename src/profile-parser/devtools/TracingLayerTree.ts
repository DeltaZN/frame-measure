...
import type {LayerPaintEvent} from './TimelineFrameModel';
import {Layer, LayerTreeBase, StickyPositionConstraint} from "./sdk/LayerTreeBase";
import {Target} from "./sdk/Target";
import {PaintProfilerModel, SnapshotWithRect} from "./sdk/PaintProfiler";
import {DOM, LayerTree} from "./generated/protocol";
import {DOMNode} from "./sdk/DOMModel";

export class TracingLayerTree extends LayerTreeBase {
  private tileById: Map<string, TracingLayerTile>;
  private paintProfilerModel: PaintProfilerModel|null;

  constructor(target: Target|null) {
    super(target);
    this.tileById = new Map();
    this.paintProfilerModel = target && target.model(PaintProfilerModel);
  }

  async setLayers(root: TracingLayerPayload|null, layers: TracingLayerPayload[]|null, paints: LayerPaintEvent[]):
      Promise<void> {
    const idsToResolve = new Set<DOM.BackendNodeId>();
    if (root) {
  ...
  pictureForRasterTile(tileId: string): Promise<SnapshotWithRect|null> {
    const tile = this.tileById.get('cc::Tile/' + tileId);
    if (!tile) {
     console.error(`Tile ${tileId} is missing`);
      return Promise.resolve(null) as Promise<SnapshotWithRect|null>;
    }
    const layer = (this.layerById(tile.layer_id) as TracingLayer | null);
    if (!layer) {
     console.error(`Layer ${tile.layer_id} for tile ${tileId} is not found`);
      return Promise.resolve(null) as Promise<SnapshotWithRect|null>;
    }
  ...
  private innerSetLayers(oldLayersById: Map<string|number, Layer>, payload: TracingLayerPayload):
      TracingLayer {
  ...
  private extractNodeIdsToResolve(
      nodeIdsToResolve: Set<DOM.BackendNodeId>, seenNodeIds: Object, payload: TracingLayerPayload): void {
...
export class TracingLayer implements Layer {
  private parentLayerId: string|null;
  private parentInternal: Layer|null;
  private layerId: string;
  private nodeInternal: DOMNode|null;
  private offsetXInternal: number;
  private offsetYInternal: number;
  private widthInternal: number;
  private heightInternal: number;
  private childrenInternal: Layer[];
  private quadInternal: number[];
  private scrollRectsInternal: LayerTree.ScrollRect[];
  ...
  private paintProfilerModel: PaintProfilerModel|null;
  constructor(paintProfilerModel: PaintProfilerModel|null, payload: TracingLayerPayload) {
  ...
  addChild(childParam: Layer): void {
  ...
  setNode(node: DOMNode|null): void {
  ...
  nodeForSelfOrAncestor(): DOMNode|null {
    let layer: (Layer|null)|this = this;
  ...
  pictureForRect(targetRect: number[]): Promise<SnapshotWithRect|null> {
  ...
  private scrollRectsFromParams(params: number[], type: LayerTree.ScrollRectType):
  ...
  private createScrollRects(payload: TracingLayerPayload): void {
    const nonPayloadScrollRects: LayerTree.ScrollRect[] = [];
    if (payload.non_fast_scrollable_region) {
      nonPayloadScrollRects.push(this.scrollRectsFromParams(
          payload.non_fast_scrollable_region, 'NonFastScrollable' as LayerTree.ScrollRectType));
    }
    if (payload.touch_event_handler_region) {
      nonPayloadScrollRects.push(this.scrollRectsFromParams(
          payload.touch_event_handler_region, LayerTree.ScrollRectType.TouchEventHandler));
    }
    if (payload.wheel_event_handler_region) {
      nonPayloadScrollRects.push(this.scrollRectsFromParams(
          payload.wheel_event_handler_region, LayerTree.ScrollRectType.WheelEventHandler));
    }
    if (payload.scroll_event_handler_region) {
      nonPayloadScrollRects.push(this.scrollRectsFromParams(
          payload.scroll_event_handler_region, LayerTree.ScrollRectType.RepaintsOnScroll));
    }
