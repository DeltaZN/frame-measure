...
import {ObjectWrapper} from "./core/Object";
import {Target} from "./sdk/Target";
import {AsyncEvent, TracingModel, TracingModelEvent} from "./TracingModel";
import {TimelineFrame, TimelineFrameModel} from "./TimelineFrameModel";
import {TimelineModelImpl, TrackType} from "./TimelineModel";
import {TimelineUIUtils} from "./TimelineUIUtils";

export class PerformanceModel extends ObjectWrapper<EventTypes> {
  private mainTargetInternal: Target|null;
  ...
  private readonly timelineModelInternal: TimelineModelImpl;
  private readonly frameModelInternal: TimelineFrameModel;
  ...
  constructor() {
    ...
    this.frameModelInternal = new TimelineFrameModel(
  ...
  setMainTarget(target: Target): void {
  ...
  // isVisible(event: TracingModelEvent): boolean {
  ...
  setTracingModel(model: TracingModel): void {
    ...
    let inputEvents: AsyncEvent[]|null = null;
    let animationEvents: AsyncEvent[]|null = null;
  ...
  addExtensionEvents(title: string, model: TracingModel, timeOffset: number): void {
  ...
  timelineModel(): TimelineModelImpl {
  ...
  // filmStripModelFrame(frame: TimelineFrame): SDK.FilmStripModel.Frame|null {
