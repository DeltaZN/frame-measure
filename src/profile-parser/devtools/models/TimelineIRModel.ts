...
import {AsyncEvent, TracingModelEvent} from "../TracingModel";
import {Segment, SegmentedRange} from "../core/SegmentedRange";
...
const eventToPhase = new WeakMap<TracingModelEvent, Phases>();

export class TimelineIRModel {
  private segments!: Segment<Phases>[];
  private drags!: SegmentedRange<Phases>;
  private cssAnimations!: SegmentedRange<Phases>;
  private responses!: SegmentedRange<Phases>;
  private scrolls!: SegmentedRange<Phases>;

  constructor() {
    this.reset();
  }

  static phaseForEvent(event: TracingModelEvent): Phases|undefined {
    return eventToPhase.get(event);
  }

  populate(inputLatencies: AsyncEvent[]|null, animations: AsyncEvent[]|null): void {
    this.reset();
    if (!inputLatencies) {
      return;
    }
    ...
    const range = new SegmentedRange<Phases>();
  ...
  private processInputLatencies(events: AsyncEvent[]): void {
    ...
    for (let i = 0; i < events.length; ++i) {
      ...
      switch (type) {
        ...
        case eventTypes.FlingStart:
          if (flingStart) {
           console.error(UIStrings.twoFlingsAtTheSameTimeSVsS + flingStart.startTime + event.startTime);
            break;
          }
          flingStart = event;
          break;
        ...
        case eventTypes.TouchStart:
          ...
          if (touchStart) {
            console.error(UIStrings.twoTouchesAtTheSameTimeSVsS + touchStart.startTime, + event.startTime);
            break;
          }
          touchStart = event;
    ...
    function canMerge(
        threshold: number, first: AsyncEvent, second: AsyncEvent): boolean {
  ...
  private setPhaseForEvent(asyncEvent: AsyncEvent, phase: Phases): void {
    eventToPhase.set(asyncEvent.steps[0], phase);
  }

  interactionRecords(): Segment<Phases>[] {
  ...
  reset(): void {
    ...
    this.segments = [];
    this.drags = new SegmentedRange(merge.bind(null, thresholdsMs.mouse));
    this.cssAnimations = new SegmentedRange(merge.bind(null, thresholdsMs.animation));
    this.responses = new SegmentedRange(merge.bind(null, 0));
    this.scrolls = new SegmentedRange(merge.bind(null, thresholdsMs.animation));

    function merge(
        threshold: number, first: Segment<Phases>,
        second: Segment<Phases>): Segment<Phases>|null {
